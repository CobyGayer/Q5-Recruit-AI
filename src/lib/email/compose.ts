...
export function buildGmailComposeUrl({ to, bcc, subject, body }: ComposeParams): string {
...
export function buildOutlookComposeUrl({ to, bcc, subject, body }: ComposeParams): string {
  const params = new URLSearchParams();
  if (to) params.set("to", to);
  if (bcc?.length) params.set("bcc", bcc.join(","));
